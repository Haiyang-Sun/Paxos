...
public class PaxosProposerHeartBeatMessage extends PaxosMessage {
	private Proposer sender;
	public PaxosProposerHeartBeatMessage(Proposer node) {
		/*
	...
	public PaxosProposerHeartBeatMessage(Proposer node, int msgId) {
		super(node, -1, msgId);
		sender = node;
	}
	...
	public ByteBuffer getMessageBytes(){
		...
		bytes.put(PaxosMessenger.msgType2Byte(MessageType.MSG_PROPOSER_HEARTBEAT));
